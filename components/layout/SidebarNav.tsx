...
const NAV_ITEMS = [
  { href: '/relatorio', icon: '📝', label: 'Relatório', implemented: true, expandable: true },
...
export function SidebarNav({ nome, gaepCodigo, perfil }: SidebarNavProps) {
  ...
  const relatorioSubItems = [
    { href: '/relatorio', icon: '📝', label: 'Novo Registro' },
    { href: '/relatorio/historico', icon: '📋', label: 'Histórico' },
  ] as const
      ...
      >
        ...
        <div style={{ padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: 4, flex: 1, overflowY: 'auto' }}>
          {visible.map((item) => {
            ...
            const isRelatorio = item.href === '/relatorio'
            if (!item.implemented) {
              return (
                <div
                  key={item.href}
            ...
            return (
              <div key={item.href}>
                {isRelatorio ? (
                  <button
                    onClick={() => setRelatorioExpanded((prev) => !prev)}
                    style={{
                      width: '100%',
                      display: 'flex',
                      alignItems: 'center',
                  ...
                  >
                    ...
                    <span style={{ fontSize: '0.75rem', color: '#64748b' }}>{relatorioExpanded || active ? '▾' : '▸'}</span>
                  </button>
                ) : isGestao ? (
                  <button
                ...
                {(relatorioExpanded || active) && isRelatorio && (
